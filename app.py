...
def load_model():
    if os.path.exists(MODEL_PATH):
        try:
            return joblib.load(MODEL_PATH), "Loaded from model_a.pkl"
...
with col_result:
    ...
    # ฟังก์ชันทำนายราคา (กรณีมีโมเดลจริง vs โมเดลจำลอง)
    def calculate_price(df):
        if model is not None:
            # หากมีโมเดล .pkl จริง (ต้องเตรียมคอลัมน์ให้ตรงกับตอน fit)
            try:
        ...
        loc_mult = {"CBD / ใจกลางเมือง": 2.2, "แนวรถไฟฟ้า (BTS/MRT)": 1.6, "ต่างจังหวัดหัวเมืองเศรษฐกิจ": 1.1, "ชานเมืองรอบนอก (Suburban)": 0.9}
        ...
        # ปรับตามอายุและสิ่งอำนวยความสะดวก
    ...
    # กราฟ Radar หรือ Waterfall เปรียบเทียบสัดส่วนมูลค่า
    breakdown_labels = ["ตัวบ้านหลัก", "ที่ดิน", "ห้อง/ฟังก์ชัน", "สิ่งอำนวยความสะดวกพิเศษ"]
    breakdown_vals = [
        living_area * 35000,
        land_area * 25000,
        (bedrooms * 120000) + (bathrooms * 90000) + (parking * 100000),
        (750000 if has_pool else 0) + (220000 if has_solar else 0)
    ]
